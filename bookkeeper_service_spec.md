...
| Поле | Тип | Описание |
|---|---|---|
| `id` | VARCHAR (PK) | ИНН или сгенерированный slug |
...
| `PUT` | `/transactions/{id}` | Редактирование. Поддерживает поле `receipt_id` (UUID или null) для ручной привязки/отвязки чека. При привязке: `reconciled_status → MATCHED`, создаётся запись `audit_log` с `action=MATCH`. При отвязке: статус сбрасывается в `UNMATCHED` только если был `MATCHED`; `IGNORED_BY_USER` и `NOT_REQUIRED` сохраняются. |
...
### 9.1 Дашборд (`/`)
- KPI-карточки: расходы за месяц, кол-во активных счетов, кол-во несверенных транзакций, кол-во конфликтов.
- Таблица **«Остатки на счетах»** с итоговой строкой.
- Таблица **«Типы расходов»**: группировка по `expense_type_id` за выбранный месяц, столбцы «Операций» и «Сумма», строка **«Итого»** внизу.
...
- Клик по строке → детальная карточка чека с позициями и привязкой документа.

### 9.4 Правила (`/rules`)
- Таблица правил классификации с фильтрами по **«Тип затрат»** и **«Приоритет»** (клиентская фильтрация).
- Кнопки создания правила и пакетного применения к диапазону дат.

### 9.5 Сверка (`/reconciliation`)
- Кнопка **«Запустить сверку»** → `POST /reconciliation/run`.
- KPI-карточки: авто-совпадений, без чека, нематчед чеки, коллизии.
- Секция **«Транзакции без чека»**: столбцы Дата, Вид расхода, Сумма; выпадающий список для выбора чека + кнопка **«Сопоставить»**; кнопка **«Игнорировать»**; пагинация (20 на странице).
- Секция **«Чеки без транзакции»**: столбцы Дата, Контрагент, Сумма; выпадающий список для выбора транзакции + кнопка **«Сопоставить»**; пагинация (20 на странице).
- Секция **«Коллизии»**: карточки с описанием коллизий.
- После каждой операции «Сопоставить» или «Игнорировать» сверка перезапускается автоматически, страница обновляется.

### 9.6 Настройки (`/settings`)
- CRUD для типов трат с тогглом `receipt_required`.
- CRUD для счетов (включая `zero_balance`).
- Управление API-ключами.
- Настройки сверки (`RECONCILE_AUTO_MATCH_MAX_HOURS`, `RECONCILE_AMOUNT_TOLERANCE`).
