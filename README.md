...
| Метод | Путь | Описание |
|-------|------|----------|
| `POST` | `/documents` | Загрузить документ (PDF/изображение); дедупликация по SHA-256 |
| `GET` | `/documents` | Список документов; фильтр по типу (`?document_type=`) |
...
| `GET` | `/receipts` | Список чеков текущего пользователя |
| `GET` | `/receipts/{id}` | Получить чек по ID |
| `PUT` | `/receipts/{id}` | Обновить чек |
| `DELETE` | `/receipts/{id}` | Удалить чек (запрещено, если привязан к транзакции) |

**Привязка контрагента к чеку** (`POST /receipts`):

| Сценарий | Передаваемые поля | Поведение |
|----------|------------------|-----------|
| Контрагент уже есть в справочнике | `counterparty_id` | Валидация существования; 404 если не найден |
| Контрагент новый, известен ИНН | `counterparty_inn` + `counterparty_name` | Поиск по ИНН; автосоздание если не найден |
| Контрагент не указан | — | `counterparty_id = null` |

`PUT /receipts/{id}` принимает `counterparty_id` из справочника (аналогично созданию).
...
| Метод | Путь | Описание |
|-------|------|----------|
| `GET` | `/transactions` | Список транзакций; фильтры по счёту, дате, типу, статусу сверки |
...
| Метод | Путь | Описание |
|-------|------|----------|
| `GET/POST` | `/accounts` | Список / создать банковский счёт |
...
Контрагент чека указывается через `counterparty_id` — ссылку на запись в справочнике «Контрагенты». Свободная строка с именем не поддерживается. При создании чека по фискальным данным (агент, импорт) контрагент может быть автоматически создан при наличии `counterparty_inn` и `counterparty_name` (ИНН валидируется: 10 или 12 цифр). Имя без ИНН игнорируется. В UI отображается `name` из справочника.
